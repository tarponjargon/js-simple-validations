...
function FormValidator(form) {
  ...
  this.getValidationFields = function(form = self.form) {
    ...
    self.getFields(form).forEach(field => {
      try {
        ...
        let isRequired =
          vtypes && vtypes.toLowerCase().indexOf("require") !== -1;
  ...
  this.valid = function(e, form = self.form) {
    try {
      ...
      if (
        ...
        try {
          let debouncedCallback = util.debounce(
            window[validCallback],
            cfg.debounceDefault
          );
          debouncedCallback(e, form, "valid");
        } catch (e) {
          console.error(
            "Problem executing valid callback on form:",
            validCallback,
            e
          );
        }
  ...
  this.invalid = function(e, form = self.form) {
    try {
      ...
      if (button && cfg.useTooltip && cfg.buttonTooltip) {
      ...
      if (
        button &&
        cfg.buttonSuccess &&
        button.classList.contains(cfg.buttonSuccess)
      ) {
      ...
      if (
        button &&
        cfg.buttonOriginalText &&
        button.getAttribute(cfg.buttonOriginalText)
      ) {
      ...
      if (
        ...
        try {
          ...
          console.error(
            "Problem executing invalid callback on form:",
            invalidCallback,
            e
          );
        }
